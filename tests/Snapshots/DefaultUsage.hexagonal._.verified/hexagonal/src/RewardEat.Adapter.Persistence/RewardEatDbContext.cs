using RewardEat.Adapter.Persistence.Entities;
...
public class RewardEatDbContext : DbContext, IUnitOfWork
{
    private readonly IEventDispatcher _domainEventDispatcher;
    private readonly ILogger<RewardEatDbContext> _logger;

    public RewardEatDbContext(
        DbContextOptions<RewardEatDbContext> options,
        IEventDispatcher domainEventDispatcher,
        ILogger<RewardEatDbContext> logger) : base(options)
    {
